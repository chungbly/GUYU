...
import { Link, SendToBack } from 'lucide-react';
import Image from 'next/image';
import { useRouter } from 'next/navigation';
import { useState } from 'react';

interface ApplicationLevelPageProps {
  paragraphStructure: Record<string, number>;
  rearrangeWordsStructure: Record<string, number>;
}
function ApplicationLevelPage({ paragraphStructure, rearrangeWordsStructure }: ApplicationLevelPageProps) {
  const router = useRouter();
  const [destination, setDestination] = useState<string | null>(null);
  const [letter, setLetter] = useState<string | null>(null);
  const structure = destination?.includes('sap-xep-cau') ? rearrangeWordsStructure : paragraphStructure;
  return (
        ...
        <div className="max-w-4xl mx-auto grid gap-6 md:grid-cols-2">
          <motion.div whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }}>
            <Card
              ...
              onClick={() => {
                setDestination('/van-dung/sap-xep-cau');
              }}
            >
          ...
          <motion.div whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }}>
            <Card
              className="group cursor-pointer overflow-hidden"
              onClick={() => {
                setDestination('/van-dung/hoan-thanh-doan-van');
              }}
            >
              <CardContent className="p-0">
                <div className="relative aspect-[4/3] overflow-hidden">
...
export default ApplicationLevelPage;
