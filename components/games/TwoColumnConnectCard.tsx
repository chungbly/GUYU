...
const initialFlashcardPairs = [
  { id: 1, left: 'Apple', right: 'Quả táo' },
  { id: 2, left: 'Banana', right: 'Quả chuối' },
  { id: 3, left: 'Orange', right: 'Quả cam' },
  { id: 4, left: 'Grape', right: 'Quả nho' },
  { id: 5, left: 'Mango', right: 'Quả xoài' },
];

export default function TwoColumnConnectFlashcards() {
  const [leftCards, setLeftCards] = useState<typeof initialFlashcardPairs>([]);
  const [rightCards, setRightCards] = useState<typeof initialFlashcardPairs>([]);
  ...
  const shuffleCards = () => {
    const shuffledLeft = [...initialFlashcardPairs].sort(() => Math.random() - 0.5);
    const shuffledRight = [...initialFlashcardPairs].sort(() => Math.random() - 0.5);
  ...
  const isGameComplete = leftCards.length === 0 && rightCards.length === 0;
  ...
  return (
    <div className="container mx-auto p-4 max-w-4xl">
      ...
      <div className="flex justify-between items-center mb-4">
        <p>
          Score: {score} / {initialFlashcardPairs.length}
        </p>
      ...
      <div className="grid grid-cols-2 gap-8">
          ...
          {leftCards.map((card, index) => (
            ...
            >
              <CardContent className="flex items-center justify-center h-16 p-2">
                <p className="text-center">{card.left}</p>
              </CardContent>
            </Card>
          ))}
        </div>
        <div className="space-y-4">
          {rightCards.map((card, index) => (
            ...
            >
              <CardContent className="flex items-center justify-center h-16 p-2">
                <p className="text-center">{card.right}</p>
              </CardContent>
            </Card>
          ))}
        </div>
